...
const EXLIB = [
  ...
  ["assistpull", "어시스트 풀업", "vpull", 2.5, false, true], ["sapull", "스트레이트암 풀다운", "vpull", 1.25],
...
].map(([id, name, pattern, inc, bw, assist]) => ({ id, name, pattern, inc, bw: !!bw, assist: !!assist }));
...
const slot = (tier, id, sets, lo, hi, rest, note) => {
  const l = LIBMAP[id];
  return { id, tier, name: l.name, pattern: l.pattern, inc: l.inc, bw: l.bw, assist: l.assist, sets, lo, hi, rest, note: note || "" };
};
  ...
  sessions: {
    upperA: {
      name: "상체 A", sub: "가슴 · 수평 밀기", ex: [
        ...
        slot("T3", "pallof", 2, 12, 15, 45, "코어 항회전"),
      ],
    },
    upperB: {
      name: "상체 B", sub: "등 너비 · 수직 당기기", ex: [
        t1("assistpull", 180, "보조중량은 작을수록 무겁다"),
    ...
    lower: {
      name: "하체", sub: "머신 중심", ex: [
        t1("hack", 240, "스트렝스 앵커"),
        slot("T2", "rdl", 3, 8, 12, 150, "러닝 있는 주엔 실패까지 가지 말 것"),
        slot("T2", "seatcurl", 3, 10, 12, 120, "무겁게"),
        slot("T2", "legpress", 2, 12, 15, 120, "발 낮게 = 사두 강조"),
        slot("T3", "lyingcurl", 2, 12, 15, 75),
...
const bwKgOf = (d) => (d.settings.bw ? toKg(d.settings.bw, d.settings.bwU || "kg") : 0);
/* 어시스트 머신은 숫자가 클수록 쉬워진다. 유효 부하 = 체중 − 보조중량 */
const effKg = (s, assist, bw) => (assist ? Math.max(0, bw - setKg(s)) : setKg(s));
const showW = (s, u) => {
...
const lookupEx = (id, custom) =>
  LIBMAP[id] || (custom || []).find((c) => c.id === id) || { id, name: id, pattern: "", inc: 2.5, bw: false, assist: false };
...
function Today({ data, patch, tenK, date, setDate, startRest }) {
  ...
  const exList = useMemo(() => {
    ...
    return session.ex.map((s) => {
      const sw = swap[s.id];
      const base = sw
        ? (() => { const l = lookupEx(sw, data.custom); return { ...s, id: l.id, name: l.name, inc: l.inc, bw: l.bw, assist: l.assist, swapped: true }; })()
        : { ...s };
    ...
    <>
      ...
      {exList.map((e) => (
        <Exercise key={e.slotId} ex={e} log={liftLog[e.id] || []} date={date} allLog={data.log}
          ...
          bodyWeight={bwKgOf(data) ? nice(fromKg(bwKgOf(data), unitOf(data, e.id)), unitOf(data, e.id)) : 0} />
      ))}
...
function Exercise({ ex, log, date, allLog, unit, onChange, onSwap, startRest, autoRest, onAddSet, onRemoveSet, tm, onSetTm, tmSrc, bodyWeight }) {
  const U = UL(unit);
  const count = Math.max(ex.sets, log.length);
  const wave = ex.wave;
  /* T1 은 세트마다 목표 중량이 다르다. 원판 단위로 반올림해서 보여준다.
     어시스트 머신은 숫자가 클수록 쉬워지므로 체중에서 목표 유효부하를 빼서 보조중량을 낸다 */
  const target = (i) => {
    if (!wave || !wave[i] || tm <= 0) return null;
    const eff = tm * wave[i].pct;
    if (!ex.assist) return roundTo(eff, ex.inc);
    if (!bodyWeight) return null;
    return Math.max(0, roundTo(bodyWeight - eff, ex.inc));
  };
  ...
  return (
    <div className={`tp-row ${doneCount === count ? "done" : ""}`}>
      ...
      {wave ? (
        ex.assist && !bodyWeight ? (
          <div className="tp-prev">어시스트 머신은 체중을 알아야 보조중량을 계산합니다. 설정 탭에서 체중을 먼저 넣으세요.</div>
        ) : tm > 0 ? (
  ...
  const byEx = useMemo(() => {
    ...
    Object.keys(data.log).sort().forEach((d) => {
      ...
      Object.entries(lift).forEach(([id, sets]) => {
        ...
        /* 보조중량을 그대로 쓰면 강해질수록 수치가 내려간다 */
        const assist = !!lookupEx(id, data.custom).assist;
        const load = (s) => effKg(s, assist, bw);
        const bestKg = Math.max(...done.map((s) => e1rm(load(s), Number(s.r))));
        const topKg = Math.max(...done.map(load));
        const volKg = done.reduce((a, s) => a + load(s) * Number(s.r), 0);
...
function Program({ data, patch }) {
  ...
  const onPick = (res) => {
    patch((d) => {
      ...
      if (pick.add) arr.push({ id, tier: "T3", name: l.name, pattern: l.pattern, inc, bw: l.bw, assist: l.assist, sets: 3, lo: 8, hi: 12, rest: 120, note: "" });
      else arr[pick.idx] = { ...arr[pick.idx], id, name: l.name, pattern: l.pattern, inc, bw: l.bw, assist: l.assist };
    });
    setPick(null);
  };
